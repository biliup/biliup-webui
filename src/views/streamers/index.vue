...
const handleDeleteItem = product => {
  ElMessageBox.confirm(
    product ? `确认删除后${product.remark}, 且无法恢复` : "",
    "提示",
    {
      type: "warning"
    }
...
const handkeOk = product => {
  ...
  createStreamers(product).then(() => {
    console.log("创建成功", product);
    getCardListData();
  });
};
</script>
      ...
      <el-empty
        v-show="
          productList
            ...
            .filter(v =>
              v.remark.toLowerCase().includes(searchValue.toLowerCase())
            ).length === 0
        "
